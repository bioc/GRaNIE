...
For more details, see also the [Package Details](GRaNIE_packageDetails.html#output_TF_peak).
...
Let's now check some diagnostic plots for the enhancer-gene connections. In analogy to the other diagnostic plots that we encountered already before, we describe their interpretation and meaning in more detail in the [Package Details](GRaNIE_packageDetails.html#output_peak_gene)!
...
The `GRaNIE` package also offers a function to visualize a filtered `eGRN` network! It is very easy to invoke, but provides many options to customize the output and the way the graph is drawn. We recommend to explore the options in the R help (`?getGRNConnections`), and here just run the default visualization. By default, `eGRNs` with 500 edges or less can be plotted with the default options, which works for this example dataset. However, this can be customized to also draw larger networks (see the parameter `maxEdgesToPlot` and the notes below):
...
**Non surprisingly, visualizing a larger network can be more challenging. If your visualization does not work or it looks not as expected, check out the [Package Details](GRaNIE_packageDetails.html#network-visualization) for recommendations.**
