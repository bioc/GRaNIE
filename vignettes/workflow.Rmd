...
require(GRaNIEData)
(files = list.files(pattern = "*",system.file("extdata", package = "GRaNIEData"),full.names = TRUE))
file_peaks = files[grep("countsATAC.75k.tsv.gz", files)]
file_RNA  = files[grep("countsRNA.sampled.tsv.gz", files)]
file_sampleMetadata = files[grep("metadata.sampled.tsv", files)]
folder_TFBS_first50 = files[grep("TFBS_selected", files)]
...
# For the sake of simplicity, we only take a subset of all samples here to speed-up the vignette code
# countsRNA.df = countsRNA.df[1:10000,1:10]
# countsPeaks.df = countsPeaks.df[1:20000,1:10]
...
Multiple output plots are produced, with two files for each of the specified `types`. Usually, this is the RNA-Seq counts (as specified with `rna` here) as well as the enhancer counts (`peaks`). For each of them, PCA plots are produced for both `raw` and `normalized` data. With `raw`, we here denote the original counts as given as input with the `addData()` function, irrespective of whether this was already pre-normalized or not. For more details, see the [Package Details Vignette](packageDetails.html). The `topn` argument specifies the number of top variable features to do PCA for - here 500.

Here, let's just take a look at one example page from one of the files that are produced, namely the original counts from the RNA-seq counts:


Let's check the screeplot first, which summarizes the PCA:
```{r, echo=FALSE, fig.cap="<i>Screeplot</i>", out.width = '80%'}
knitr::include_graphics("figs/PCA_RNA_raw/p-02.png")
```

Another summary i to correlate all available metadata with the PCs and check fo correlation, which can help to identify potentially confounding variables that explain a large proportion of the observed variability:
```{r, echo=FALSE, fig.cap="<i>Metadata correlation plot</i>", out.width = '80%'}
knitr::include_graphics("figs/PCA_RNA_raw/p-03.png")
```

Lastly, for a selected metadata (here, the gender, labeled as `sex`), let's take a look at PC1 and PC2:
```{r, echo=FALSE, fig.cap="<i>PCA plots with colored metadata</i>", out.width = '80%'}
knitr::include_graphics("figs/PCA_RNA_raw/p-17.png")
```

For all details and further comments on how to understand and interpret the plots, see the [Package Details Vignette](packageDetails.html).


## Add TFs and TFBS and overlap with enhancers

Now it is time to add data for TFs and predicted TF binding sites (TFBS)! Our `GRaNIE` package requires pre-computed TFBS that need to be in a specific format (see the [Package Details Vignette](packageDetails.html) for details). In brief, a 6-column bed file must be present for each TF, with a specific file name that starts with the name of the TF, an arbitrary and optional suffix (here: "_TFBS") and a particular file ending (supported are `bed` or `bed.gz`; here, we specify the latter). All these files must be located in a particular folder that the `addTFBS()` functions then searches in order to identify those files that match the specified patterns. We provide example TFBS for the 3 genome assemblies we support, see the comment below and the [Package Details Vignette](packageDetails.html) for details. After setting this up, we are ready to overlap the TFBS and the enhancers by calling the function `overlapPeaksAndTFBS()`.

For more parameter details, see the R help (`?addTFBS` and `?overlapPeaksAndTFBS`). 

```{r addTFBS, echo=TRUE, include=TRUE, eval = TRUE, class.output="scroll-200", results='hold'}
...
Optionally, we can filter both enhancers and RNA-Seq data according to various criteria using the function `filterData()`. 

For the open chromatin enhancers, we currently support three filters: 

1. Filter by their normalized mean read counts (`minNormalizedMean_peaks`, default 5)
2. Filter by their size / width (in bp) and discarding enhancers that exceed a particular threshold (`maxSize_peaks`, default: 10000 bp)
...
The default values are usually suitable for bulk data and should result in the removal of very few enhancers / genes; however, for single-cell data, lowering them may more reasonable. The output will print clearly how many enhancers and genes have been filtered, so you can rerun the function with different values if needed.
...
# Chromosomes to keep for enhancers. This should be a vector of chromosome names
...
```
We can see from the output that no enhancers have been filtered due to their size and almost 11,000 have been filtered due to their small mean read counts, which collectively leaves around 64,000 enhancers out of 75,000 originally. For the RNA data, almost half of the data has been filtered (16,211 out of around 35,000 genes). 
...
GRN = addConnections_TF_peak(GRN, plotDiagnosticPlots = TRUE, 
                                     connectionTypes = c("expression"),
                                     corMethod = "pearson", forceRerun = TRUE)
```
From the output, we see that a total of 65 TFs have RNA-seq data available and consequently will be included and correlated with the enhancer accessibility. The created PDF files are mentioned in the output and these we'll take a look at now!

## Quality control 2: Diagnostic plots for TF-enhancer connections

After adding the TF-enhancer links to our `GRaNIE` object, let's look at some diagnostic plots. The `plots` folder within the specified output folder when initializing the `GRaNIE` object should now contain two new files that are named `TF_peak.fdrCurves_original.pdf` and `TF_peak.fdrCurves_permuted.pdf`. For reasons of brevity and organization, we fully describe their interpretation and meaning in detail in the [Package Details Vignette](packageDetails.html#output_TF_peak) and not here, however.
...
Here, in summary, a few positively correlated *E2F6.0.A* - enhancer pairs (with a correlation of at least 0.5 or so) are statistically significant and may be retained for the final *eGRN* network (if corresponding genes connecting to the respective enhancers are found).
...
```{r runARClassification, echo=TRUE, include=TRUE, eval = TRUE, class.output="scroll-200"}
...
```
From the output, we see that the classification has been run for both real and permuted data, as before. The contents of these plots are identical to and uses in fact practically the same code as our `diffTF` software. We refer to the following links for more details:
...
The function produces different output plots and summaries of the method, but we here focus on only one of them, the summary heatmap, while the other plots are discussed in the [Package Details Vignette](packageDetails.html#output_AR) only.


Let's first take a look at the random data:

```{r, echo=FALSE, fig.cap="<i>Summary heatmap for the AR classification for permuted data</i>", out.width = '60%'}
knitr::include_graphics("figs/AR_heatmap_expr_perm/p-2.png")
```

For the real, non-permuted data, it looks like this:
```{r, echo=FALSE, fig.cap="<i>Summary heatmap for the AR classification for real data</i>", out.width = '60%'}
knitr::include_graphics("figs/AR_heatmap_expr_real/p-2.png")
```

In summary, we can appreciate that for permuted data, almost all TFs are reassuringly classified as `undetermined` (that is expected, given the random nature of the data), while for the non-permuted one, the majority of TFs is either an `activator` or `repressor`. This is irrespective of the classification stringency. Overall, this indicates we capture real signal.
...
We see from the output that almost 42,000 enhancer-gene links have been identified that match our parameters. From these 42,000, however, only around 18,804 actually had corresponding RNA-seq data available, while RNA-seq data was missing or has been filtered for the other. This is a rather typical case, as not all known and annotated genes are included in the RNA-seq data in the first place. Similar to before, the correlations have also been performed for the permuted data.


## Quality control 3: Diagnostic plots for enhancer-gene connections

Let's now check some diagnostic plots for the enhancer-gene connections. In analogy to the other diagnostic plots that we encountered already before, we describe their interpretation and meaning in more detail in the [Package Details Vignette](packageDetails.html) and here only focus on a few overall details!


The following plot summarizes many key QC measures we implemented:
```{r, echo=FALSE, fig.cap="<i>Diagnostic plots (QC) for the enhancer-gene connections</i>", out.width = '80%'}
knitr::include_graphics("figs/peakGene_QC_all/p-01.png")
```

Without explaining the details here, we can see from mainly the upper left plot that the enhancer-gene connections look good indeed!  
...
Let's start with a connection summary in form of a heatmap! There are 3 heatmaps, one for TFs, enhancers (labeled peaks) and genes, respectively. All of them compare the number of distinct TFs, enhancers, and genes that end up in the final `eGRN` in dependence of how stringently the connections are filtered (i.e., different FDR thresholds for both TF-enhancers and enhancer-genes). In addition, the same is repeated for the permuted data, which enables you to judge the connectivity of the real data as compared to what you can expect with random data!

For TFs, it looks like this, and we see that the numbers are generally very small because we just run the analysis with few TFs. For the permuted data, depending on the random nature of it, none or almost none connections survive the filtering. You should see much bigger differences for full TF data and not just a few selected ones:
```{r, echo=FALSE, fig.cap="<i>Filtered eGRN connection summary (heatmap) for TFs</i>", out.width = '75%'}
knitr::include_graphics("figs/connectionsHeatmap/p-1.png")
```

The plots for enhancers and genes show more dramatic differences, however. It shows that the TF-enhancer FDR has to be set to at least 0.2; otherwise, almost no connections survive the filtering:
```{r, echo=FALSE, fig.cap="<i>Filtered eGRN connection summary (heatmap) for enhancers</i>", out.width = '75%'}
knitr::include_graphics("figs/connectionsHeatmap/p-2.png")
```

```{r, echo=FALSE, fig.cap="<i>Filtered eGRN connection summary (heatmap) for genes</i>", out.width = '75%'}
...
```{r, echo=FALSE, fig.cap="<i>Filtered eGRN connection summary (boxplot)</i>", out.width = '80%'}
knitr::include_graphics("figs/connectionsBoxplot/p-01.png")
```
...
```{r, echo=FALSE, fig.cap="<i>Filtered eGRN connection summary (boxplot)</i>", out.width = '80%'}
knitr::include_graphics("figs/connectionsBoxplot/p-11.png")
```
...
Many results are produces by this convenience function, and we here show only a few of them. For a description of all them, we refer to the [Package Details Vignette](packageDetails.html). 

Let's run an enrichment analysis using `GO` (here, only *Biological Process*). We also support enrichment using *Disease Ontology* (abbreviated `DO`) as well as *Reactome* and `KEGG`.
...
Let's start with checking some general network statistics for the full network. First we can check the vertex distribution and gene types for the overall network to get an idea of how the network looks like:
```{r, echo=FALSE, fig.cap="<i>General network statistics for the filtered network</i>", out.width = '80%'}
knitr::include_graphics("figs/stats.overall/p-1.png")
```

We plot the network statistics for both the TF-enhancer-gene as well as TF-gene network, and we here show the results for the TF-gene network only:

First, we see the vertex degree of TF and genes, respectively:
```{r, echo=FALSE, fig.cap="<i>General network statistics for the filtered network</i>", out.width = '80%'}
knitr::include_graphics("figs/stats.overall/p-5.png")
```
...
```{r, echo=FALSE, fig.cap="<i>Centrality network statistics for the filtered network</i>", out.width = '80%'}
knitr::include_graphics("figs/stats.overall/p-6.png")
```

```{r, echo=FALSE, fig.cap="<i>Centrality network statistics for the filtered network</i>", out.width = '80%'}
knitr::include_graphics("figs/stats.overall/p-7.png")
```


We can also check these statistics community-specifically, but we omit these plots here and refer to the [Package Details Vignette](packageDetails.html). 


Instead, let's take a look at `GO` term enrichment results for the full network:

```{r, echo=FALSE, fig.cap="<i>GO term enrichment results for the full network</i>", out.width = '100%'}
knitr::include_graphics("figs/enrichment.overall/p-1.png")
```

Even though this is just example data for only a few TF, we see some interesting enrichments already. The biological plausibility of them and how to continue after is now your challenge!


Lastly, and probably often most interestingly, we also offer community-specific enrichment analyses, and we here show some examples:

Excitingly, the two biggest communities (1 and 5) show very different enrichments:
```{r, echo=FALSE, fig.cap="<i>GO term enrichment results for individual communities</i>", out.width = '100%'}
knitr::include_graphics("figs/enrichment.community/p-01.png")
```
...
When checking the `KEGG` database, we also find some enriched terms:
```{r, echo=FALSE, fig.cap="<i>GO term enrichment results for individual communities</i>", out.width = '100%'}
#knitr::include_graphics("figs/enrichment.community/p-09.png")
```

We also provide an overview across the whole network and all communities that lists all the enriched terms that appear in at least one enrichment analysis, so a direct comparison of the specificity and commonalities across communities and between the general network and any community is facilitated. This also shows that some terms, here more than with a full dataset, are only identified as being enriched for the full network but not within any of the communities individually:
```{r, echo=FALSE, fig.cap="<i>Enrichment summary and comparison across communities and the whole network</i>", out.width = '100%'}
#knitr::include_graphics("figs/enrichment.community/p-07.png")
```

A filtered version of this only for the top 10 enriched terms is also available:
```{r, echo=FALSE, fig.cap="<i>Enrichment summary and comparison across communities and the whole network</i>", out.width = '100%'}
#knitr::include_graphics("figs/enrichment.community/p-08.png")
```

We do not show it here, but all enrichment analyses (including the summaries) are also available for the GO ontology `Molecular Function` (abbreviated `GO_MF` in the plots) in addition to `Biological Process` (`GO_BP`).

**In addition to `GO`, we also support additional enrichment analyses (`KEGG`, `Disease Ontology`, and `Reactome`), which can selected in the same way as you have seen in the code chunk above! More are coming soon, stay tuned!**
