...
# When the GRaNIEData package is installed, the following code can be used:
...
# Temporarily, we load the data directly from the web without the GRaNIEData dependency:
...
GRN = plot_stats_connectionSummary(GRN, type = "heatmap", outputFolder = "output/plots", forceRerun = TRUE)
GRN = plot_stats_connectionSummary(GRN, type = "boxplot", outputFolder = "output/plots", forceRerun = TRUE)
```
...
```{r enrichment, echo=TRUE, include=TRUE, eval = FALSE, class.output="scroll-200"}
GRN = performAllNetworkAnalyses(GRN, ontology = c("GO_BP"), outputFolder = "output/plots", forceRerun = TRUE)
```
