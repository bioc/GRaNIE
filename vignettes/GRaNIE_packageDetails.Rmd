...
-   **Genome assembly and annotation packages** (only one of the four is
    ...
    -   **all of the following packages are ONLY needed for either
        ...
        -   `BSgenome.Mmusculus.UCSC.mm39`,
            `TxDb.Mmusculus.UCSC.mm39.knownGene`: Needed only for `mm39`
...
**Our recommended and default TF database is HOCOMOCO v12, see below for
details.**

### HOCOMOCO-derived TFBS and download links

As a default, we use HOCOMOCO-derived TFBS predictions that were
predicted with `PWMScan`. For further method details, see the [diffTF
paper from our
lab](https://difftf.readthedocs.io/en/latest/projectInfo.html#citation).

We provide TFBS predictions for HOCOMOCO-based TF motifs that were used
with `PWMScan` for `hg19`, `hg38`, `mm10` and soon `mm39`. As of 2024, we recommend
using HOCOMOCO v12 ([Website](https://hocomoco12.autosome.org),
[Paper](https://academic.oup.com/nar/article/52/D1/D154/7424436)). Using
...
-   **`PWMScan` + HOCOMOCO v12 INVIVO collection** ([Download
    here](https://s3.embl.de/zaugg-web/GRaNIE/TFBS/hg38/PWMScan_HOCOMOCOv12_H12INVIVO.tar.gz))
-   `PWMScan` + HOCOMOCO v11 ([Download
    here](https://s3.embl.de/zaugg-web/GRaNIE/TFBS/hg38/PWMScan_HOCOMOCOv11.tar.gz))
-   `FIMO` + HOCOMOCO v11 ([Download
    here](https://s3.embl.de/zaugg-web/GRaNIE/TFBS/hg38/FIMO_HOCOMOCOv11.tar.gz))

**hg19:**

-   `PWMScan` + HOCOMOCO v10 ([Download
    here](https://s3.embl.de/zaugg-web/GRaNIE/TFBS/hg19/PWMScan_HOCOMOCOv10.tar.gz))

**mm10:**

-   **`PWMScan` + HOCOMOCO v12 INVIVO collection** ([Download
    here](https://s3.embl.de/zaugg-web/GRaNIE/TFBS/mm10/PWMScan_HOCOMOCOv12_H12INVIVO.tar.gz))
-   `PWMScan` + HOCOMOCO v10 ([Download
    here](https://s3.embl.de/zaugg-web/GRaNIE/TFBS/mm10/PWMScan_HOCOMOCOv10.tar.gz))
    
**mm39: (COMING SOON, links do not yet work)**

-   **`PWMScan` + HOCOMOCO v12 INVIVO collection** ([Download
    here](https://s3.embl.de/zaugg-web/GRaNIE/TFBS/mm39/PWMScan_HOCOMOCOv12_H12INVIVO.tar.gz))
-   `PWMScan` + HOCOMOCO v10 ([Download
    here](https://s3.embl.de/zaugg-web/GRaNIE/TFBS/mm39/PWMScan_HOCOMOCOv10.tar.gz))

To use them, simply download the file, extract and then reference the
folder within the `motifFolder` argument in the `addTFBS()` function -
see also the [Workflow Vignette](GRaNIE_workflow.html#add-tfs-and-tfbs-and-overlap-with-peak)
for examples.
...
-   we provide example files for selected genome assemblies (*hg19*,
    *hg38*,  *mm9*, *mm10*,  *mm39*) that are fully compatible with `GRaNIE` as
    separate downloads. For more information, check the links above.
...
R/Bioconductor: `JASPAR` 2022 and 2024. While we so far tested only with
the `JASPAR2022` database / R package, the other ones should also work -
if not, please let us know. Using `JASPAR2024` as TF database is much
...
from our (limited) test experience. For details, see the `addTFBS()` and
`overlapPeaksAndTFBS()` functions.

Note that we also implemented an option to customize which TFBS
collection to use from the `JASPAR` database via the function argument
`JASPAR_useSpecificTaxGroup` from `addTFBS()`. For example, this feature
can be used to specify the whole \code{vertebrates} TF collection
...
of using `JASPAR` motives and cannot currently comment much on it except
...
3.  If other known promoter-enhancer links are provided, these are taken
    ...
    the parameter `overlapTypeGene` in analogy to how it works
    everywhere else.
...
-   inside both real and background links (i.e., within
    ...
    `peak.ID`, `gene.ENSEMBL`, `peak_gene.distance`), their connection
    origin (e.g `TADs`, `knownLinks` or `neighborhood`), and statistical
    properties are stored (`peak_gene.r`, `peak_gene.p_raw`).
