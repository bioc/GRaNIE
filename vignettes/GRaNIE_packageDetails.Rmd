...
Note that peaks must not overlap. If they do, an informative error
...
After a filtered `eGRN` has been stored in the GRN object (in
`GRN@connections$all.filtered[["0"]]`), the user can run all network
related analysis such as enrichment or visualization. See the next
sections for details.

## Constructing the `eGRN` network

After a filtered set of TF-peak-gene links (i.e., a full `eGRN`) has been
build with `filterGRNAndConnectGenes()`, the actual graph must be
...
using more than 100,000 to 200,000 or so peaks as the memory footprint
as well as running time may otherwise increase notably.

Given that our approach is a correlation-based one, it seems preferable
