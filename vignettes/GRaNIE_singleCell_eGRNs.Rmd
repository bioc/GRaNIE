...
-   as we usually use the **mean count per cluster** in the preprocessing from above, set **`normalization_peaks` and `normalization_rna` both to `none`** and therefore basically treat the data as pre-normalized. This may not be the only reasonable choice related to data normalization, but is currently the recommended way.
